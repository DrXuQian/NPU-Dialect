func.func @vgg16(
  ...
  // VGG block 2: 3 convs, 256 channels
  ...
  %init55 = tensor.empty() : tensor<1x256x28x28xf16>
  %fill56 = linalg.fill ins(%cst : f16) outs(%init55 : tensor<1x256x28x28xf16>) -> tensor<1x256x28x28xf16>
  %conv57 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<2> : tensor<2xi64>
  } ins(%pad54, %w_b2_pool : tensor<1x256x58x58xf16>, tensor<256x256x3x3xf16>)
    outs(%fill56 : tensor<1x256x28x28xf16>) -> tensor<1x256x28x28xf16>
  %empty58 = tensor.empty() : tensor<1x256x28x28xf16>
  %relu59 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
                     affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>],
    iterator_types = ["parallel", "parallel", "parallel", "parallel"]
  } ins(%conv57 : tensor<1x256x28x28xf16>)
    outs(%empty58 : tensor<1x256x28x28xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  // VGG block 3: 3 convs, 512 channels
  ...
  %conv63 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%pad60, %w_b3_c0 : tensor<1x256x30x30xf16>, tensor<512x256x3x3xf16>)
  ...
  %conv69 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%pad66, %w_b3_c1 : tensor<1x512x30x30xf16>, tensor<512x512x3x3xf16>)
  ...
  %conv75 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%pad72, %w_b3_c2 : tensor<1x512x30x30xf16>, tensor<512x512x3x3xf16>)
  ...
  %conv81 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<2> : tensor<2xi64>
  } ins(%pad78, %w_b3_pool : tensor<1x512x30x30xf16>, tensor<512x512x3x3xf16>)
  ...
  // VGG block 4: 3 convs, 512 channels
  %pad84 = tensor.pad %relu83 low[0, 0, 1, 1] high[0, 0, 1, 1] {
  ^bb0(%a0: index, %a1: index, %a2: index, %a3: index):
    tensor.yield %cst : f16
  } : tensor<1x512x14x14xf16> to tensor<1x512x16x16xf16>
  %init85 = tensor.empty() : tensor<1x512x14x14xf16>
  %fill86 = linalg.fill ins(%cst : f16) outs(%init85 : tensor<1x512x14x14xf16>) -> tensor<1x512x14x14xf16>
  %conv87 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%pad84, %w_b4_c0 : tensor<1x512x16x16xf16>, tensor<512x512x3x3xf16>)
    outs(%fill86 : tensor<1x512x14x14xf16>) -> tensor<1x512x14x14xf16>
  %empty88 = tensor.empty() : tensor<1x512x14x14xf16>
  %relu89 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
                     affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>],
    iterator_types = ["parallel", "parallel", "parallel", "parallel"]
  } ins(%conv87 : tensor<1x512x14x14xf16>)
    outs(%empty88 : tensor<1x512x14x14xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  %conv93 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%pad90, %w_b4_c1 : tensor<1x512x16x16xf16>, tensor<512x512x3x3xf16>)
  ...
  %conv99 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%pad96, %w_b4_c2 : tensor<1x512x16x16xf16>, tensor<512x512x3x3xf16>)
  ...
  %conv105 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<2> : tensor<2xi64>
  } ins(%pad102, %w_b4_pool : tensor<1x512x16x16xf16>, tensor<512x512x3x3xf16>)
  ...
  %init108 = tensor.empty() : tensor<1x4096x1x1xf16>
  %fill109 = linalg.fill ins(%cst : f16) outs(%init108 : tensor<1x4096x1x1xf16>) -> tensor<1x4096x1x1xf16>
  %fc1110 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%relu107, %w_fc1 : tensor<1x512x7x7xf16>, tensor<4096x512x7x7xf16>)
    outs(%fill109 : tensor<1x4096x1x1xf16>) -> tensor<1x4096x1x1xf16>
  %empty111 = tensor.empty() : tensor<1x4096x1x1xf16>
  %relu112 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
                     affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>],
    iterator_types = ["parallel", "parallel", "parallel", "parallel"]
  } ins(%fc1110 : tensor<1x4096x1x1xf16>)
    outs(%empty111 : tensor<1x4096x1x1xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  %init113 = tensor.empty() : tensor<1x4096x1x1xf16>
  %fill114 = linalg.fill ins(%cst : f16) outs(%init113 : tensor<1x4096x1x1xf16>) -> tensor<1x4096x1x1xf16>
  %conv115 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%relu112, %w_fc2 : tensor<1x4096x1x1xf16>, tensor<4096x4096x1x1xf16>)
    outs(%fill114 : tensor<1x4096x1x1xf16>) -> tensor<1x4096x1x1xf16>
  %empty116 = tensor.empty() : tensor<1x4096x1x1xf16>
  %relu117 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
                     affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>],
    iterator_types = ["parallel", "parallel", "parallel", "parallel"]
  } ins(%conv115 : tensor<1x4096x1x1xf16>)
    outs(%empty116 : tensor<1x4096x1x1xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  %init118 = tensor.empty() : tensor<1x1000x1x1xf16>
  %fill119 = linalg.fill ins(%cst : f16) outs(%init118 : tensor<1x1000x1x1xf16>) -> tensor<1x1000x1x1xf16>
  %conv120 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%relu117, %w_fc3 : tensor<1x4096x1x1xf16>, tensor<1000x4096x1x1xf16>)
    outs(%fill119 : tensor<1x1000x1x1xf16>) -> tensor<1x1000x1x1xf16>
  return %conv120 : tensor<1x1000x1x1xf16>
}
