func.func @fcn_resnet50(
    ...
    %w_dec0: tensor<512x2048x3x3xf16>,
    %w_dec1: tensor<256x512x3x3xf16>,
    %w_out: tensor<21x256x1x1xf16>) -> tensor<1x21x16x16xf16> {
  ...
  // enc8: 3x3 s2 1024->2048 32x32
  %pad48 = tensor.pad %relu47 low[0, 0, 1, 1] high[0, 0, 1, 1] {
  ^bb0(%a0: index, %a1: index, %a2: index, %a3: index):
    tensor.yield %cst : f16
  } : tensor<1x1024x32x32xf16> to tensor<1x1024x34x34xf16>
  %init49 = tensor.empty() : tensor<1x2048x16x16xf16>
  %fill50 = linalg.fill ins(%cst : f16) outs(%init49 : tensor<1x2048x16x16xf16>) -> tensor<1x2048x16x16xf16>
  %conv51 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<2> : tensor<2xi64>
  } ins(%pad48, %w_enc8 : tensor<1x1024x34x34xf16>, tensor<2048x1024x3x3xf16>)
    outs(%fill50 : tensor<1x2048x16x16xf16>) -> tensor<1x2048x16x16xf16>
  %empty52 = tensor.empty() : tensor<1x2048x16x16xf16>
  %relu53 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
                     affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>],
    iterator_types = ["parallel", "parallel", "parallel", "parallel"]
  } ins(%conv51 : tensor<1x2048x16x16xf16>)
    outs(%empty52 : tensor<1x2048x16x16xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  } -> tensor<1x2048x16x16xf16>
  // enc9: 3x3 s1 2048->2048 16x16
  %pad54 = tensor.pad %relu53 low[0, 0, 1, 1] high[0, 0, 1, 1] {
  ^bb0(%a0: index, %a1: index, %a2: index, %a3: index):
    tensor.yield %cst : f16
  } : tensor<1x2048x16x16xf16> to tensor<1x2048x18x18xf16>
  %init55 = tensor.empty() : tensor<1x2048x16x16xf16>
  %fill56 = linalg.fill ins(%cst : f16) outs(%init55 : tensor<1x2048x16x16xf16>) -> tensor<1x2048x16x16xf16>
  %conv57 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%pad54, %w_enc9 : tensor<1x2048x18x18xf16>, tensor<2048x2048x3x3xf16>)
    outs(%fill56 : tensor<1x2048x16x16xf16>) -> tensor<1x2048x16x16xf16>
  %empty58 = tensor.empty() : tensor<1x2048x16x16xf16>
  %relu59 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
                     affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>],
    iterator_types = ["parallel", "parallel", "parallel", "parallel"]
  } ins(%conv57 : tensor<1x2048x16x16xf16>)
    outs(%empty58 : tensor<1x2048x16x16xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  %conv63 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%pad60, %w_dec0 : tensor<1x2048x18x18xf16>, tensor<512x2048x3x3xf16>)
  ...
  // dec1: 3x3 s1 512->256 16x16
  %pad66 = tensor.pad %relu65 low[0, 0, 1, 1] high[0, 0, 1, 1] {
  ^bb0(%a0: index, %a1: index, %a2: index, %a3: index):
    tensor.yield %cst : f16
  } : tensor<1x512x16x16xf16> to tensor<1x512x18x18xf16>
  %init67 = tensor.empty() : tensor<1x256x16x16xf16>
  %fill68 = linalg.fill ins(%cst : f16) outs(%init67 : tensor<1x256x16x16xf16>) -> tensor<1x256x16x16xf16>
  %conv69 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%pad66, %w_dec1 : tensor<1x512x18x18xf16>, tensor<256x512x3x3xf16>)
    outs(%fill68 : tensor<1x256x16x16xf16>) -> tensor<1x256x16x16xf16>
  %empty70 = tensor.empty() : tensor<1x256x16x16xf16>
  %relu71 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
                     affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>],
    iterator_types = ["parallel", "parallel", "parallel", "parallel"]
  } ins(%conv69 : tensor<1x256x16x16xf16>)
    outs(%empty70 : tensor<1x256x16x16xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  // Output: 1x1 256->21
  %init72 = tensor.empty() : tensor<1x21x16x16xf16>
  %fill73 = linalg.fill ins(%cst : f16) outs(%init72 : tensor<1x21x16x16xf16>) -> tensor<1x21x16x16xf16>
  %conv74 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%relu71, %w_out : tensor<1x256x16x16xf16>, tensor<21x256x1x1xf16>)
    outs(%fill73 : tensor<1x21x16x16xf16>) -> tensor<1x21x16x16xf16>
  return %conv74 : tensor<1x21x16x16xf16>
}
