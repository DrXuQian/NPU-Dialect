func.func @wide_resnet50_2(
  ...
  %init255 = tensor.empty() : tensor<1x1024x14x14xf16>
  %fill256 = linalg.fill ins(%cst : f16) outs(%init255 : tensor<1x1024x14x14xf16>) -> tensor<1x1024x14x14xf16>
  %conv257 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%relu254, %w_s3_b0_c1 : tensor<1x1024x14x14xf16>, tensor<1024x1024x1x1xf16>)
    outs(%fill256 : tensor<1x1024x14x14xf16>) -> tensor<1x1024x14x14xf16>
  %empty258 = tensor.empty() : tensor<1x1024x14x14xf16>
  %relu259 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
                     affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>],
    iterator_types = ["parallel", "parallel", "parallel", "parallel"]
  } ins(%conv257 : tensor<1x1024x14x14xf16>)
    outs(%empty258 : tensor<1x1024x14x14xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  %init261 = tensor.empty() : tensor<1x1024x7x7xf16>
  %fill262 = linalg.fill ins(%cst : f16) outs(%init261 : tensor<1x1024x7x7xf16>) -> tensor<1x1024x7x7xf16>
  %conv263 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<2> : tensor<2xi64>
  } ins(%pad260, %w_s3_b0_c2 : tensor<1x1024x16x16xf16>, tensor<1024x1024x3x3xf16>)
    outs(%fill262 : tensor<1x1024x7x7xf16>) -> tensor<1x1024x7x7xf16>
  %empty264 = tensor.empty() : tensor<1x1024x7x7xf16>
  %relu265 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
                     affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>],
    iterator_types = ["parallel", "parallel", "parallel", "parallel"]
  } ins(%conv263 : tensor<1x1024x7x7xf16>)
    outs(%empty264 : tensor<1x1024x7x7xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  %conv268 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%relu265, %w_s3_b0_c3 : tensor<1x1024x7x7xf16>, tensor<2048x1024x1x1xf16>)
  ...
  %init276 = tensor.empty() : tensor<1x1024x7x7xf16>
  %fill277 = linalg.fill ins(%cst : f16) outs(%init276 : tensor<1x1024x7x7xf16>) -> tensor<1x1024x7x7xf16>
  %conv278 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%relu275, %w_s3_b1_c1 : tensor<1x2048x7x7xf16>, tensor<1024x2048x1x1xf16>)
    outs(%fill277 : tensor<1x1024x7x7xf16>) -> tensor<1x1024x7x7xf16>
  %empty279 = tensor.empty() : tensor<1x1024x7x7xf16>
  %relu280 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
                     affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>],
    iterator_types = ["parallel", "parallel", "parallel", "parallel"]
  } ins(%conv278 : tensor<1x1024x7x7xf16>)
    outs(%empty279 : tensor<1x1024x7x7xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  %init282 = tensor.empty() : tensor<1x1024x7x7xf16>
  %fill283 = linalg.fill ins(%cst : f16) outs(%init282 : tensor<1x1024x7x7xf16>) -> tensor<1x1024x7x7xf16>
  %conv284 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%pad281, %w_s3_b1_c2 : tensor<1x1024x9x9xf16>, tensor<1024x1024x3x3xf16>)
    outs(%fill283 : tensor<1x1024x7x7xf16>) -> tensor<1x1024x7x7xf16>
  %empty285 = tensor.empty() : tensor<1x1024x7x7xf16>
  %relu286 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
                     affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>],
    iterator_types = ["parallel", "parallel", "parallel", "parallel"]
  } ins(%conv284 : tensor<1x1024x7x7xf16>)
    outs(%empty285 : tensor<1x1024x7x7xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  %conv289 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%relu286, %w_s3_b1_c3 : tensor<1x1024x7x7xf16>, tensor<2048x1024x1x1xf16>)
  ...
  // Bottleneck 2048->2048 mid=1024 7x7 stride=1
  %init294 = tensor.empty() : tensor<1x1024x7x7xf16>
  %fill295 = linalg.fill ins(%cst : f16) outs(%init294 : tensor<1x1024x7x7xf16>) -> tensor<1x1024x7x7xf16>
  %conv296 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%relu293, %w_s3_b2_c1 : tensor<1x2048x7x7xf16>, tensor<1024x2048x1x1xf16>)
    outs(%fill295 : tensor<1x1024x7x7xf16>) -> tensor<1x1024x7x7xf16>
  %empty297 = tensor.empty() : tensor<1x1024x7x7xf16>
  %relu298 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
                     affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>],
    iterator_types = ["parallel", "parallel", "parallel", "parallel"]
  } ins(%conv296 : tensor<1x1024x7x7xf16>)
    outs(%empty297 : tensor<1x1024x7x7xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  %init300 = tensor.empty() : tensor<1x1024x7x7xf16>
  %fill301 = linalg.fill ins(%cst : f16) outs(%init300 : tensor<1x1024x7x7xf16>) -> tensor<1x1024x7x7xf16>
  %conv302 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%pad299, %w_s3_b2_c2 : tensor<1x1024x9x9xf16>, tensor<1024x1024x3x3xf16>)
    outs(%fill301 : tensor<1x1024x7x7xf16>) -> tensor<1x1024x7x7xf16>
  %empty303 = tensor.empty() : tensor<1x1024x7x7xf16>
  %relu304 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
                     affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>],
    iterator_types = ["parallel", "parallel", "parallel", "parallel"]
  } ins(%conv302 : tensor<1x1024x7x7xf16>)
    outs(%empty303 : tensor<1x1024x7x7xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  %conv307 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%relu304, %w_s3_b2_c3 : tensor<1x1024x7x7xf16>, tensor<2048x1024x1x1xf16>)
