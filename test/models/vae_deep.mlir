func.func @vae_deep(
    ...
    %w_dec0: tensor<64x128xf16>,
    %w_dec1: tensor<128x256xf16>,
    %w_dec2: tensor<256x512xf16>,
    %w_dec3: tensor<512x1024xf16>,
    %w_dec4: tensor<1024x1024xf16>) -> tensor<1x1024xf16> {
  ...
  // === Decoder ===
  // dec0: 64->128
  %init25 = tensor.empty() : tensor<1x128xf16>
  %fill26 = linalg.fill ins(%cst : f16) outs(%init25 : tensor<1x128xf16>) -> tensor<1x128xf16>
  %mm27 = linalg.matmul ins(%relu24, %w_dec0 : tensor<1x64xf16>, tensor<64x128xf16>)
                          outs(%fill26 : tensor<1x128xf16>) -> tensor<1x128xf16>
  %empty28 = tensor.empty() : tensor<1x128xf16>
  %relu29 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>,
                     affine_map<(d0, d1) -> (d0, d1)>],
    iterator_types = ["parallel", "parallel"]
  } ins(%mm27 : tensor<1x128xf16>)
    outs(%empty28 : tensor<1x128xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  // dec1: 128->256
  %init30 = tensor.empty() : tensor<1x256xf16>
  %fill31 = linalg.fill ins(%cst : f16) outs(%init30 : tensor<1x256xf16>) -> tensor<1x256xf16>
  %mm32 = linalg.matmul ins(%relu29, %w_dec1 : tensor<1x128xf16>, tensor<128x256xf16>)
                          outs(%fill31 : tensor<1x256xf16>) -> tensor<1x256xf16>
  %empty33 = tensor.empty() : tensor<1x256xf16>
  %relu34 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>,
                     affine_map<(d0, d1) -> (d0, d1)>],
    iterator_types = ["parallel", "parallel"]
  } ins(%mm32 : tensor<1x256xf16>)
    outs(%empty33 : tensor<1x256xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  // dec2: 256->512
  %init35 = tensor.empty() : tensor<1x512xf16>
  %fill36 = linalg.fill ins(%cst : f16) outs(%init35 : tensor<1x512xf16>) -> tensor<1x512xf16>
  %mm37 = linalg.matmul ins(%relu34, %w_dec2 : tensor<1x256xf16>, tensor<256x512xf16>)
                          outs(%fill36 : tensor<1x512xf16>) -> tensor<1x512xf16>
  %empty38 = tensor.empty() : tensor<1x512xf16>
  %relu39 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>,
                     affine_map<(d0, d1) -> (d0, d1)>],
    iterator_types = ["parallel", "parallel"]
  } ins(%mm37 : tensor<1x512xf16>)
    outs(%empty38 : tensor<1x512xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  // dec3: 512->1024
  %init40 = tensor.empty() : tensor<1x1024xf16>
  %fill41 = linalg.fill ins(%cst : f16) outs(%init40 : tensor<1x1024xf16>) -> tensor<1x1024xf16>
  %mm42 = linalg.matmul ins(%relu39, %w_dec3 : tensor<1x512xf16>, tensor<512x1024xf16>)
                          outs(%fill41 : tensor<1x1024xf16>) -> tensor<1x1024xf16>
  %empty43 = tensor.empty() : tensor<1x1024xf16>
  %relu44 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>,
                     affine_map<(d0, d1) -> (d0, d1)>],
    iterator_types = ["parallel", "parallel"]
  } ins(%mm42 : tensor<1x1024xf16>)
    outs(%empty43 : tensor<1x1024xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  // dec4: 1024->1024
  %init45 = tensor.empty() : tensor<1x1024xf16>
  %fill46 = linalg.fill ins(%cst : f16) outs(%init45 : tensor<1x1024xf16>) -> tensor<1x1024xf16>
  %mm47 = linalg.matmul ins(%relu44, %w_dec4 : tensor<1x1024xf16>, tensor<1024x1024xf16>)
                          outs(%fill46 : tensor<1x1024xf16>) -> tensor<1x1024xf16>
  return %mm47 : tensor<1x1024xf16>
}
