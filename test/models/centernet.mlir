func.func @centernet(
    ...
    %w8: tensor<256x512x3x3xf16>,
    %w9: tensor<128x256x3x3xf16>,
    %w_det: tensor<80x128x1x1xf16>) -> tensor<1x80x32x32xf16> {
  %cst = arith.constant 0.0 : f16

  // conv0: 7x7 s2 3->64 512x512
  ...
  } ins(%pad0, %w0 : tensor<1x3x518x518xf16>, tensor<64x3x7x7xf16>)
  ...
  %conv9 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<2> : tensor<2xi64>
  } ins(%pad6, %w1 : tensor<1x64x258x258xf16>, tensor<64x64x3x3xf16>)
  ...
  } ins(%pad12, %w2 : tensor<1x64x130x130xf16>, tensor<128x64x3x3xf16>)
  ...
  %conv21 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%pad18, %w3 : tensor<1x128x130x130xf16>, tensor<128x128x3x3xf16>)
  ...
  %conv27 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<2> : tensor<2xi64>
  } ins(%pad24, %w4 : tensor<1x128x130x130xf16>, tensor<256x128x3x3xf16>)
  ...
  %conv33 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%pad30, %w5 : tensor<1x256x66x66xf16>, tensor<256x256x3x3xf16>)
  ...
  // conv6: 3x3 s2 256->512 64x64
  %pad36 = tensor.pad %relu35 low[0, 0, 1, 1] high[0, 0, 1, 1] {
  ^bb0(%a0: index, %a1: index, %a2: index, %a3: index):
    tensor.yield %cst : f16
  } : tensor<1x256x64x64xf16> to tensor<1x256x66x66xf16>
  %init37 = tensor.empty() : tensor<1x512x32x32xf16>
  %fill38 = linalg.fill ins(%cst : f16) outs(%init37 : tensor<1x512x32x32xf16>) -> tensor<1x512x32x32xf16>
  %conv39 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<2> : tensor<2xi64>
  } ins(%pad36, %w6 : tensor<1x256x66x66xf16>, tensor<512x256x3x3xf16>)
    outs(%fill38 : tensor<1x512x32x32xf16>) -> tensor<1x512x32x32xf16>
  %empty40 = tensor.empty() : tensor<1x512x32x32xf16>
  %relu41 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
                     affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>],
    iterator_types = ["parallel", "parallel", "parallel", "parallel"]
  } ins(%conv39 : tensor<1x512x32x32xf16>)
    outs(%empty40 : tensor<1x512x32x32xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  %conv45 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%pad42, %w7 : tensor<1x512x34x34xf16>, tensor<512x512x3x3xf16>)
  ...
  %conv51 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%pad48, %w8 : tensor<1x512x34x34xf16>, tensor<256x512x3x3xf16>)
  ...
  // conv9: 3x3 s1 256->128 32x32
  %pad54 = tensor.pad %relu53 low[0, 0, 1, 1] high[0, 0, 1, 1] {
  ^bb0(%a0: index, %a1: index, %a2: index, %a3: index):
    tensor.yield %cst : f16
  } : tensor<1x256x32x32xf16> to tensor<1x256x34x34xf16>
  %init55 = tensor.empty() : tensor<1x128x32x32xf16>
  %fill56 = linalg.fill ins(%cst : f16) outs(%init55 : tensor<1x128x32x32xf16>) -> tensor<1x128x32x32xf16>
  %conv57 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%pad54, %w9 : tensor<1x256x34x34xf16>, tensor<128x256x3x3xf16>)
    outs(%fill56 : tensor<1x128x32x32xf16>) -> tensor<1x128x32x32xf16>
  %empty58 = tensor.empty() : tensor<1x128x32x32xf16>
  %relu59 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
                     affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>],
    iterator_types = ["parallel", "parallel", "parallel", "parallel"]
  } ins(%conv57 : tensor<1x128x32x32xf16>)
    outs(%empty58 : tensor<1x128x32x32xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  // Detection head: 1x1 128->80
  %init60 = tensor.empty() : tensor<1x80x32x32xf16>
  %fill61 = linalg.fill ins(%cst : f16) outs(%init60 : tensor<1x80x32x32xf16>) -> tensor<1x80x32x32xf16>
  %conv62 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%relu59, %w_det : tensor<1x128x32x32xf16>, tensor<80x128x1x1xf16>)
    outs(%fill61 : tensor<1x80x32x32xf16>) -> tensor<1x80x32x32xf16>
  return %conv62 : tensor<1x80x32x32xf16>
}
