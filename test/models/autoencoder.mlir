func.func @autoencoder(
  ...
  // enc3: 128->64
  %init15 = tensor.empty() : tensor<1x64xf16>
  %fill16 = linalg.fill ins(%cst : f16) outs(%init15 : tensor<1x64xf16>) -> tensor<1x64xf16>
  %mm17 = linalg.matmul ins(%relu14, %w_enc3 : tensor<1x128xf16>, tensor<128x64xf16>)
                          outs(%fill16 : tensor<1x64xf16>) -> tensor<1x64xf16>
  %empty18 = tensor.empty() : tensor<1x64xf16>
  %relu19 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>,
                     affine_map<(d0, d1) -> (d0, d1)>],
    iterator_types = ["parallel", "parallel"]
  } ins(%mm17 : tensor<1x64xf16>)
    outs(%empty18 : tensor<1x64xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  %mm22 = linalg.matmul ins(%relu19, %w_dec0 : tensor<1x64xf16>, tensor<64x128xf16>)
