func.func @deeplabv3plus_resnet50(
    ...
    %w_enc9: tensor<256x2048x1x1xf16>,
    %w_enc10: tensor<256x256x3x3xf16>,
    %w_dec0: tensor<256x256x3x3xf16>,
    %w_dec1: tensor<256x256x3x3xf16>,
    %w_dec2: tensor<128x256x3x3xf16>,
    %w_out: tensor<21x128x1x1xf16>) -> tensor<1x21x32x32xf16> {
  ...
  // enc9: 1x1 s1 2048->256 32x32
  %init54 = tensor.empty() : tensor<1x256x32x32xf16>
  %fill55 = linalg.fill ins(%cst : f16) outs(%init54 : tensor<1x256x32x32xf16>) -> tensor<1x256x32x32xf16>
  %conv56 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%relu53, %w_enc9 : tensor<1x2048x32x32xf16>, tensor<256x2048x1x1xf16>)
    outs(%fill55 : tensor<1x256x32x32xf16>) -> tensor<1x256x32x32xf16>
  %empty57 = tensor.empty() : tensor<1x256x32x32xf16>
  %relu58 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
                     affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>],
    iterator_types = ["parallel", "parallel", "parallel", "parallel"]
  } ins(%conv56 : tensor<1x256x32x32xf16>)
    outs(%empty57 : tensor<1x256x32x32xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  } -> tensor<1x256x32x32xf16>
  // enc10: 3x3 s1 256->256 32x32
  ...
  %conv62 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%pad59, %w_enc10 : tensor<1x256x34x34xf16>, tensor<256x256x3x3xf16>)
  ...
  %conv68 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%pad65, %w_dec0 : tensor<1x256x34x34xf16>, tensor<256x256x3x3xf16>)
  ...
  %conv74 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%pad71, %w_dec1 : tensor<1x256x34x34xf16>, tensor<256x256x3x3xf16>)
  ...
  // dec2: 3x3 s1 256->128 32x32
  %pad77 = tensor.pad %relu76 low[0, 0, 1, 1] high[0, 0, 1, 1] {
  ^bb0(%a0: index, %a1: index, %a2: index, %a3: index):
    tensor.yield %cst : f16
  } : tensor<1x256x32x32xf16> to tensor<1x256x34x34xf16>
  %init78 = tensor.empty() : tensor<1x128x32x32xf16>
  %fill79 = linalg.fill ins(%cst : f16) outs(%init78 : tensor<1x128x32x32xf16>) -> tensor<1x128x32x32xf16>
  %conv80 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%pad77, %w_dec2 : tensor<1x256x34x34xf16>, tensor<128x256x3x3xf16>)
    outs(%fill79 : tensor<1x128x32x32xf16>) -> tensor<1x128x32x32xf16>
  %empty81 = tensor.empty() : tensor<1x128x32x32xf16>
  %relu82 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
                     affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>],
    iterator_types = ["parallel", "parallel", "parallel", "parallel"]
  } ins(%conv80 : tensor<1x128x32x32xf16>)
    outs(%empty81 : tensor<1x128x32x32xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  // Output: 1x1 128->21
  %init83 = tensor.empty() : tensor<1x21x32x32xf16>
  %fill84 = linalg.fill ins(%cst : f16) outs(%init83 : tensor<1x21x32x32xf16>) -> tensor<1x21x32x32xf16>
  %conv85 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%relu82, %w_out : tensor<1x128x32x32xf16>, tensor<21x128x1x1xf16>)
    outs(%fill84 : tensor<1x21x32x32xf16>) -> tensor<1x21x32x32xf16>
  return %conv85 : tensor<1x21x32x32xf16>
}
