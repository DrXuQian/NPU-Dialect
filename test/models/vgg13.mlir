func.func @vgg13(
  ...
  %init49 = tensor.empty() : tensor<1x256x28x28xf16>
  %fill50 = linalg.fill ins(%cst : f16) outs(%init49 : tensor<1x256x28x28xf16>) -> tensor<1x256x28x28xf16>
  %conv51 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<2> : tensor<2xi64>
  } ins(%pad48, %w_b2_pool : tensor<1x256x58x58xf16>, tensor<256x256x3x3xf16>)
    outs(%fill50 : tensor<1x256x28x28xf16>) -> tensor<1x256x28x28xf16>
  %empty52 = tensor.empty() : tensor<1x256x28x28xf16>
  %relu53 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
                     affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>],
    iterator_types = ["parallel", "parallel", "parallel", "parallel"]
  } ins(%conv51 : tensor<1x256x28x28xf16>)
    outs(%empty52 : tensor<1x256x28x28xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  %init55 = tensor.empty() : tensor<1x512x28x28xf16>
  %fill56 = linalg.fill ins(%cst : f16) outs(%init55 : tensor<1x512x28x28xf16>) -> tensor<1x512x28x28xf16>
  %conv57 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%pad54, %w_b3_c0 : tensor<1x256x30x30xf16>, tensor<512x256x3x3xf16>)
    outs(%fill56 : tensor<1x512x28x28xf16>) -> tensor<1x512x28x28xf16>
  %empty58 = tensor.empty() : tensor<1x512x28x28xf16>
  %relu59 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
                     affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>],
    iterator_types = ["parallel", "parallel", "parallel", "parallel"]
  } ins(%conv57 : tensor<1x512x28x28xf16>)
    outs(%empty58 : tensor<1x512x28x28xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  %init61 = tensor.empty() : tensor<1x512x28x28xf16>
  %fill62 = linalg.fill ins(%cst : f16) outs(%init61 : tensor<1x512x28x28xf16>) -> tensor<1x512x28x28xf16>
  %conv63 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%pad60, %w_b3_c1 : tensor<1x512x30x30xf16>, tensor<512x512x3x3xf16>)
    outs(%fill62 : tensor<1x512x28x28xf16>) -> tensor<1x512x28x28xf16>
  %empty64 = tensor.empty() : tensor<1x512x28x28xf16>
  %relu65 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
                     affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>],
    iterator_types = ["parallel", "parallel", "parallel", "parallel"]
  } ins(%conv63 : tensor<1x512x28x28xf16>)
    outs(%empty64 : tensor<1x512x28x28xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  %conv69 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<2> : tensor<2xi64>
  } ins(%pad66, %w_b3_pool : tensor<1x512x30x30xf16>, tensor<512x512x3x3xf16>)
  ...
  %init73 = tensor.empty() : tensor<1x512x14x14xf16>
  %fill74 = linalg.fill ins(%cst : f16) outs(%init73 : tensor<1x512x14x14xf16>) -> tensor<1x512x14x14xf16>
  %conv75 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%pad72, %w_b4_c0 : tensor<1x512x16x16xf16>, tensor<512x512x3x3xf16>)
    outs(%fill74 : tensor<1x512x14x14xf16>) -> tensor<1x512x14x14xf16>
  %empty76 = tensor.empty() : tensor<1x512x14x14xf16>
  %relu77 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
                     affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>],
    iterator_types = ["parallel", "parallel", "parallel", "parallel"]
  } ins(%conv75 : tensor<1x512x14x14xf16>)
    outs(%empty76 : tensor<1x512x14x14xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  %init79 = tensor.empty() : tensor<1x512x14x14xf16>
  %fill80 = linalg.fill ins(%cst : f16) outs(%init79 : tensor<1x512x14x14xf16>) -> tensor<1x512x14x14xf16>
  %conv81 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%pad78, %w_b4_c1 : tensor<1x512x16x16xf16>, tensor<512x512x3x3xf16>)
    outs(%fill80 : tensor<1x512x14x14xf16>) -> tensor<1x512x14x14xf16>
  %empty82 = tensor.empty() : tensor<1x512x14x14xf16>
  %relu83 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
                     affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>],
    iterator_types = ["parallel", "parallel", "parallel", "parallel"]
  } ins(%conv81 : tensor<1x512x14x14xf16>)
    outs(%empty82 : tensor<1x512x14x14xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  %init85 = tensor.empty() : tensor<1x512x7x7xf16>
  %fill86 = linalg.fill ins(%cst : f16) outs(%init85 : tensor<1x512x7x7xf16>) -> tensor<1x512x7x7xf16>
  %conv87 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<2> : tensor<2xi64>
  } ins(%pad84, %w_b4_pool : tensor<1x512x16x16xf16>, tensor<512x512x3x3xf16>)
    outs(%fill86 : tensor<1x512x7x7xf16>) -> tensor<1x512x7x7xf16>
  %empty88 = tensor.empty() : tensor<1x512x7x7xf16>
  %relu89 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
                     affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>],
    iterator_types = ["parallel", "parallel", "parallel", "parallel"]
  } ins(%conv87 : tensor<1x512x7x7xf16>)
    outs(%empty88 : tensor<1x512x7x7xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  %init90 = tensor.empty() : tensor<1x4096x1x1xf16>
  %fill91 = linalg.fill ins(%cst : f16) outs(%init90 : tensor<1x4096x1x1xf16>) -> tensor<1x4096x1x1xf16>
  %fc192 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%relu89, %w_fc1 : tensor<1x512x7x7xf16>, tensor<4096x512x7x7xf16>)
    outs(%fill91 : tensor<1x4096x1x1xf16>) -> tensor<1x4096x1x1xf16>
  %empty93 = tensor.empty() : tensor<1x4096x1x1xf16>
  %relu94 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
                     affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>],
    iterator_types = ["parallel", "parallel", "parallel", "parallel"]
  } ins(%fc192 : tensor<1x4096x1x1xf16>)
    outs(%empty93 : tensor<1x4096x1x1xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  %init95 = tensor.empty() : tensor<1x4096x1x1xf16>
  %fill96 = linalg.fill ins(%cst : f16) outs(%init95 : tensor<1x4096x1x1xf16>) -> tensor<1x4096x1x1xf16>
  %conv97 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%relu94, %w_fc2 : tensor<1x4096x1x1xf16>, tensor<4096x4096x1x1xf16>)
    outs(%fill96 : tensor<1x4096x1x1xf16>) -> tensor<1x4096x1x1xf16>
  %empty98 = tensor.empty() : tensor<1x4096x1x1xf16>
  %relu99 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
                     affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>],
    iterator_types = ["parallel", "parallel", "parallel", "parallel"]
  } ins(%conv97 : tensor<1x4096x1x1xf16>)
    outs(%empty98 : tensor<1x4096x1x1xf16>) {
  ^bb0(%in: f16, %out: f16):
    %z = arith.constant 0.0 : f16
    %m = arith.maximumf %in, %z : f16
    linalg.yield %m : f16
  ...
  %init100 = tensor.empty() : tensor<1x1000x1x1xf16>
  %fill101 = linalg.fill ins(%cst : f16) outs(%init100 : tensor<1x1000x1x1xf16>) -> tensor<1x1000x1x1xf16>
  %conv102 = linalg.conv_2d_nchw_fchw {
    dilations = dense<1> : tensor<2xi64>,
    strides = dense<1> : tensor<2xi64>
  } ins(%relu99, %w_fc3 : tensor<1x4096x1x1xf16>, tensor<1000x4096x1x1xf16>)
    outs(%fill101 : tensor<1x1000x1x1xf16>) -> tensor<1x1000x1x1xf16>
  return %conv102 : tensor<1x1000x1x1xf16>
}
